...
```{r}
# No Eye-tracking
No_Eye_tracking <- update(StartlocationsrandomIntercept, .~. +   Matchf + ContextEffectf*Agent_Action_levelf )
# Model summary
...
linear_model <- lm(AbsolutError ~ Dwelling_Time_Building_Gaze + Dwelling_Time_Agent_Gaze + Experimentf + ContextEffectf*Agent_Action_levelf*Matchf, data=df)


# Complete model with all predictors
Complete_model <- update(StartlocationsrandomIntercept, 
                        . ~ . + Dwelling_Time_Building_Gaze + Dwelling_Time_Agent_Gaze + Matchf + ContextEffectf*Agent_Action_levelf)
...
library(doParallel)
cl <- makeCluster(detectCores() - 1)  # Leave one core free for other tasks
...
emm_context <- emmeans(No_Eye_tracking, ~ ContextEffectf)
emm_agent <- emmeans(No_Eye_tracking, ~ Agent_Action_levelf)
emm_interaction <- emmeans(No_Eye_tracking, ~ ContextEffectf * Agent_Action_levelf)
emm_congruency <- emmeans(No_Eye_tracking, ~ Matchf)
