...
StartlocationsrandomIntercept <-lmer(AbsolutError_Building ~ 1 + (1|ID) + (1|PointingTaskStartingLocations_Building), data = df)
```
...
```
```{r}
HumanA_Agent <- read.csv("/Volumes/TwoTeras/2_DataSets_Experiments_1_2/humanA_Behavioural_Agent.csv", sep =",")
unique(HumanA_Agent$ID)
```
```{r}
HumanA_Agent <- HumanA_Agent %>% dplyr::filter(!ID %in% c(4580, 3540))
HumanA_Agent$ContextEffectf <-dplyr::recode(HumanA_Agent$BuildingCategory_Agent, 
                                      Residential = -0.5, Public= 0.5,
                                      .default = NaN)
HumanA_Agent$AgentPresence <-dplyr::recode(HumanA_Agent$AvatarPresenceCategory_Agent,
                                             Omitted = -0.5, Present= 0.5,
                                             .default = NaN)
HumanA_Agent$Agent_Action_level <-dplyr::recode(HumanA_Agent$AgentCategory_Agent,
                                             Passive = -0.5, Active= 0.5,
                                             .default = NaN)
...
# Data descriptives
```{r}
MainVariables_Agent <- subset(HumanA_Agent, select = c(AbsolutError_Agent, RT_Agent))
...
ggplot(df_Agent,aes(x=AbsolutError_Agent,group=Agent_Action_levelf,fill=Agent_Action_levelf))+
  geom_histogram(position="identity",alpha=0.5,binwidth=10)+  facet_grid(cols = vars(ContextEffectf))+theme_bw()
```
```{r }
library(dplyr)
TwoFactorTable_Agent <- HumanA_Agent %>% 
  group_by(ContextEffectf, Agent_Action_levelf)%>%
  summarise(AccuracyMean = mean(AbsolutError_Agent, na.rm = TRUE),
            n=n(),
            AccuracyStandardDev = sd(AbsolutError_Agent, na.rm = TRUE),
            RTMean = mean(RT_Agent, na.rm = TRUE),
            RTStandardDev = sd(RT_Agent, na.rm = TRUE))
library(tidyr)
...
interceptOnly_Agent<-gls(AbsolutErrorR ~ 1, data = df_Agent, 
                    method = "ML")
IDrandomInterceptOnly_Agent <-lmer(AbsolutErrorR ~ 1 + (1|ID), data = df_Agent)
TrialIDrandomIntercept_Agent <-lmer(AbsolutErrorR ~ 1 + (1|StartPointID_Agent) + (1|ID), data = df_Agent)
```
...
df_Agent$AbsolutErrorR <- round(HumanA_Fixations$AbsolutError, digits = 3)
qqp(df_Agent$AbsolutErrorR, "norm")
qqp(df_Agent$AbsolutErrorR, "lnorm")
