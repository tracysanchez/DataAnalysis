...
HumanA <- read.csv("/Volumes/SSD/Experiments_1_2/humanA_Behavioural_Building.csv", sep =",")
```
...
```{r echo = FALSE,warning = FALSE }
HumanAf <- HumanA %>% dplyr::filter(!ID %in% c(0479, 2361, 3246, 3572, 3976, 5238, 5531, 5895, 7264))
```
...
ggplot(data=subset(TwoFactorTableUnite, !is.na(AgentPresencef)),
    ...
    geom_errorbar(aes(linetype=AgentPresencef,
                      ymin= RT_BuildingMean-RTStandardError, 
...
```{r echo=FALSE}
ggplot(data=subset(ThreeFactorTableUnite, !is.na(Agent_Action_levelf)),
       aes(x     = ContextEffectf,
           y     = AccuracyMean,
...
```{r}
TwofactorInteraction <-update(AgentActive, .~. + ContextEffectf*Agent_Action_levelf)
...
```
```{r}
TwoInteractions_P <-update(StartlocationsrandomIntercept, .~. + ContextEffectf*Agent_Action_levelf + AgentPresencef)
summary(TwoInteractions_P)
Anova(TwoInteractions_P)
```
...
```
```{r}
model <- glmmPQL(RT_Building ~  ContextEffectf*AgentPresencef, ~1|ID/PointingTaskStartingLocations_Building,  family = gaussian(link = "log"), data = HumanAf, verbose = FALSE)
summary(model)
```
...
```{r}
interceptOnlyt <-gls(log(RT_Building) ~ 1, data = df, 
                    method = "ML")
...
ActiveA <-update(MeaningfulContext, .~. + Agent_Action_levelf)
TwoInteractions_P <-update(ActiveA, .~. + ContextEffectf*Agent_Action_levelf + AgentPresencef )
